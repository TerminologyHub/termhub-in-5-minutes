...
if [ ${#arr[@]} -ne 0 ] || [ -z $token ]; then
  ...
  echo "  e.g. $0 --token \$token --query SNOMEDCT_US-ICD10CM"
...
if [ "$id" = "sample" ]; then
  id=$(curl -s -G "$url/mapset" -H "Authorization: Bearer $token" --data-urlencode "limit=1" | jq -r '.items[0].id // empty')
  if [ -z "$id" ]; then
    echo "ERROR: Unable to resolve sample mapset id"
    exit 1
  fi
fi
...
if [ ! -z $id ]; then
  target_url="$url/mapset/$id"
  curl -v -w "\n%{http_code}" -G "$target_url" -H "Authorization: Bearer $token" 2> /dev/null > /tmp/x.$$
elif [ ! -z $project ]; then
  target_url="$url/project/$project/mapset"
  curl -v -w "\n%{http_code}" -G "$target_url" -H "Authorization: Bearer $token" 2> /dev/null > /tmp/x.$$
else
  curl -v -w "\n%{http_code}" -G "$url/mapset" -H "Authorization: Bearer $token" --data-urlencode "query=$query" --data-urlencode "limit=$limit" --data-urlencode "offset=$offset" --data-urlencode "ascending=$ascending" --data-urlencode "sort=$sort" 2> /dev/null > /tmp/x.$$
fi
if [ $? -ne 0 ]; then
